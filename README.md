...
기본 탭은 월세. 필터·지역·정렬·가중치 상태는 URL 쿼리스트링에 동기화되므로
"링크 복사" 버튼(또는 주소창 복사)으로 링크만 보내면 받은 사람도 같은 화면을 본다.
...
| 세대수 | 300+ (빌라는 무관) | 500+ |
| 전용면적 | 59㎡+ | 59㎡+ |
| 가격 | 보증금 1억~4.5억, 월세 ≤150만 | 8억~14억 |
| 기타 | 쉐어하우스/방한칸/단기 제외, **신축 빌라·연립·다세대·도시형생활주택(전용 52㎡+, 준공 10년 이내·주차 가능·엘리베이터 없음 확인 시 제외) 포함** — 보증금(전세사기) 위험 자동 점검 배지 | 재건축(JGC) 포함 |
